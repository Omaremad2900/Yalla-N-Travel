...
const SellerProducts = ( ) => {
  ...
  useEffect(() => {
    fetchProducts();
  }, [sellerService, currentPage]);


 
    const fetchProducts = async () => {
      try {
        const response = await sellerService.getProductForSeller(currentPage, itemsPerPage);
          ...
          {filteredProducts.length === 0 ? (
            ...
            <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredProducts.map((product, idx) => (
                <ProductCard key={idx} product={product} loggedInUser={loggedInUser} refreshProducts={refreshProducts} />
              ))}
            </ul>
          )}
